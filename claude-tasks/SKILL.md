...
**Selecting one.** The active profile is named by `CLAUDE_TASKS_PROFILE`; unset means `personal`. In loop mode the firing's prompt names it, with the file's absolute path. Interactively, check the environment (`printenv CLAUDE_TASKS_PROFILE`): the user sets it per tree through the `env` block of a `.claude/settings.json` at the tree's root (the work checkouts' parent folder, alongside that identity's `GH_TOKEN` and git author), so a session opened there is a work session without anyone saying so. **Read the profile file at the start of every survey**, before the first `td` call, and name the active profile in the first survey report of a session. Never infer a profile from a repo, a project name, or the shape of a task: if the environment is silent, it's `personal`; the user naming the other profile in chat counts as setting it for the session.
