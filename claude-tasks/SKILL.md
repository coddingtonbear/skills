---
name: claude-tasks
description: Pick up, work, and file delegated Todoist tasks from Claude's own Todoist account for the active profile (Coddingtonbot for personal projects, Coddingtonworkbot for work) — the queue is every task assigned to that account in the projects shared with it, in-progress work wears the claude-inflight label, plans are gated with the claude-plan-required label, the claude-devenv label grants a task the user's own checkout instead of a temporary worktree, decisions and reviews go back to the user as ask subtasks assigned to them, and the user takes work back by reassigning. Use when the user says "let's look at your tasks", "get started on whatever's next", "what's on your plate", "what needs me", "pick up the next task", "add a task for this", or otherwise asks Claude to work from or write to its Todoist queue.
---

# Working my delegated Todoist task queue

The user delegates tasks to me through Todoist, using **assignment between two accounts**: theirs and mine. Which two depends on the **active profile** (next section): for personal projects I am **Coddingtonbot**; for work projects I am **Coddingtonworkbot**, a separate Todoist account, and on GitHub I act through the user's own login instead of my own. This skill defines how to find my tasks, how much autonomy each one grants, how work and decisions flow back to the user, and how to add new tasks to the queue.

All Todoist access goes through the official `td` CLI (`@doist/todoist-cli`), and **every queue operation runs as my account**: `td --user <bot> …`, where `<bot>` is the active profile's `CLAUDE_TASKS_BOT_USER` — the `td --user …` shorthand throughout this file means exactly that. The user's account is td's default, so a `td` call without `--user` acts as *them* — never do that for queue work. Always pass `--json` (or `--ndjson`) when reading, and always reference tasks, projects, and comments as `id:<id>` — a bare ref is a fuzzy title search and can silently act on the wrong task.

## Profiles: which accounts I am

Every identity this skill depends on lives in a profile file, `profiles/<name>.env` beside this file — never in the skill text. A profile is a flat, shell-sourceable list:

| Variable | Meaning |
|---|---|
| `CLAUDE_TASKS_BOT_USER`, `CLAUDE_TASKS_BOT_NAME` | my Todoist account: the `td --user` ref, the address the user shares projects with, the name an assignment shows |
| `CLAUDE_TASKS_TOKEN_VAR` | the *name* of the secrets-file entry holding that account's API token (the loop's pre-check reads it; I get my own token from `td --user <bot> auth token view`) |
| `CLAUDE_TASKS_GITHUB_MODE` | `bot` or `shared` — see GitHub identity modes |
| `CLAUDE_TASKS_GITHUB_LOGIN`, `CLAUDE_TASKS_USER_GITHUB_LOGIN` | the login I post to GitHub as, and the user's |
| `CLAUDE_TASKS_ROOT` | where this profile's checkouts live |
| `CLAUDE_TASKS_WORKSPACE` | `checkout` or `worktree` — where a task's work happens; see Workspaces |
| `CLAUDE_TASKS_WORKTREES` | where temporary worktrees go (the loop exports the resolved path) |

**Selecting one.** There is no default profile. In loop mode the firing's prompt names it, with the file's absolute path, and the loop script exports `CLAUDE_TASKS_PROFILE` to match (the script itself refuses to launch without a `--profile`). Interactively, the user names it in chat ("work profile", "your work tasks"), which sets it for the session; failing that, check the environment (`printenv CLAUDE_TASKS_PROFILE`). **If neither names one, stop and ask which profile this is before the first `td` call** — never guess, and never fall back to one profile because the other wasn't mentioned: a wrong guess is the wrong Todoist account and the wrong GitHub identity on the wrong machine (this happened on the work machine on 2026-09-10, when the old "unset means personal" rule had a work session acting as Coddingtonbot). Never infer a profile from a repo, a project name, or the shape of a task either. **Read the profile file at the start of every survey**, before the first `td` call, and name the active profile in the first survey report of a session.

The two profiles' queues never mix — each bot account sees only the projects shared with it, so a survey is per profile by construction. The vault is shared, and the loop's run notes go under `claude-loops/<profile>/`. Project resolution matches Todoist project *names*, so a name that exists in both accounts collides; that's the user's to rename before sharing the second one, and something to say in the report if it happens.

## The hard boundary: only tasks assigned to me

The shared projects hold the user's own tasks alongside mine. **A task is mine if and only if it is assigned to my account — the active profile's bot.** Anything else — unassigned, or assigned to the user — I don't work, edit, complete, or comment on, even if it looks like something I could do. The full list of things I touch:
...
- **`claude-devenv`** — a label, orthogonal to state, meaningful under the `worktree` workspace default: this task may work in the checkout itself — the user's development environment — instead of a temporary worktree (see Workspaces). It's the user's label too: I never add it (I ask for it with a `Needs unblock` ask when a task needs the environment), and I leave it in place.
...
2. **Fetch my plate.** In-repo: normalize `git remote get-url origin`, enter Project resolution by `url`, resolve the note's `todoist-project` to a project id via `td --user <bot> project list --json`, then `td --user … task list --project id:<id> --assignee me --json --full --all`. Across every project (global mode): `td --user … task list --filter "assigned to: me" --json --full --all`, grouped by project in the report. **Then sort the plate into list order yourself** — the order these calls return is not the order the user sees (a filter listing came back with a project's second task ahead of its first, 2026-09-17). Within each project, the order is the task list as shown: tasks with no section first, then each section in its own order (`td --user … section list --project id:<id> --json --full`), and within that by `childOrder`; an assigned subtask sorts at its parent's position. `childOrder` is only in `--full` output, which is why both calls pass it. Across projects there is no user-visible order to recover — my account's project order is my own sidebar, not theirs — so projects stay grouped and the choice between them follows step 6.
...
5. **Candidates are Queued tasks and answered Waiting tasks.**
   ...
   Then read each candidate's description fully, plus its comments (`td --user … comment list id:<task> --json`) — the user may still leave short notes there; a later comment supersedes the description where they conflict. A task whose description or comments say it is blocked on an incomplete task is not eligible; neither is a `claude-devenv` task while another task on the same repo holds the checkout (see Workspaces).
6. **Choose**: a handed-back task awaiting close-out goes first, before anything else — it's cheap and clears the user's plate. Otherwise default to list order, top first — the sorted order from step 2, never the order `td task list` happened to return. A task returning from Waiting (an answered ask subtask) generally goes first — the user has just spent attention on it and the context is warm. A user-set priority (p1–p3) outranks position; in the JSON `priority` runs the other way from the UI (4 is p1, 1 is no priority). Across projects, compare each project's first candidate: higher priority first, then the one with the earliest `addedAt`, so no project's queue starves behind another's. Otherwise take a task out of order only when it would genuinely be better done *after* work further down completes (it builds on, is blocked by, or would be reworked by that later task). Skipping ahead needs no confirmation, but the skip must be called out — in the chat report *and* the dev log — with the rationale.
7. **Stale check.** An In progress task with no ask subtask, note edit, or commit from me in over a day is probably an abandoned session. Don't silently skip it: report it, and offer to resume it (read its task note/`Phase:` line and comments, then continue) before starting anything Queued. If its `Phase:` block still names a workspace, the abandoned session left it held: restoring the checkout, or removing a worktree whose branch was pushed, is the first step of resuming (see Workspaces).
...
The same survey and lifecycle run in an ordinary session — the user opens Claude Code (in a repo, or in the profile's root — `~/Documents/Projects` for personal — for the whole plate) and says "let's get started on your claude tasks". One task or round per ask; then report and stop, and take the user's next instruction: "continue" (next candidate by the usual rules), "what's on your plate", or **a task named in chat** — which counts as the user releasing it whether or not it's assigned yet (chat *is* their control surface): self-assign it on pickup so Todoist reflects the release, and say so in the report. The user may `/compact` or `/clear` between tasks freely: nothing carries over turns that isn't also in Todoist or the vault. Running interactively while the loop script is also running is safe — the `Phase:` line is the claim, so the two never take the same task, and under the `worktree` workspace default they never share a tree either (see Workspaces) — but the user should expect the script to grab the next Queued task while they're mid-conversation.

### Global mode: working across every project

When the session starts from a folder that contains the checkouts rather than from one of them — the profile's `CLAUDE_TASKS_ROOT` (`~/Documents/Projects` for personal) is the intended root; `~/` also works but puts the whole home directory in scope and makes unanchored searches slow — or the user asks to look across projects:

1. **Fetch the whole plate**: `td --user … task list --filter "assigned to: me" --json --full --all`, sorted into list order per project (step 2 above); group the report by project. The user may narrow it in chat ("just the obsidian projects") — that's a filter on the report and candidate order, not a change to what's mine.
...
4. **Work in the task's workspace with absolute paths** — the checkout, or a temporary worktree of it, per Workspaces — `git -C`, `npm --prefix`, absolute file paths, and every Glob/Grep anchored under `<path>` — rather than changing directory, so nothing depends on the session's cwd. Everything else in the lifecycle is unchanged: the project note is the one found in step 2, task notes are its peers, branches come off that repo's `main`.
...
Global mode is only reliable when the checkouts are inside the session's working directory (or were added with `--add-dir`); from an unrelated folder every file access will prompt. If that happens, say so and suggest restarting from the profile's root instead of fighting through prompts.
...
- **`url`** — the repository behind the project, when one exists, in normalized HTTPS form (`git@github.com:owner/repo` ≡ `https://github.com/owner/repo`; no trailing `.git`). Enter here when starting from a checkout: normalize `git remote get-url origin` and match. The field also implies where the checkout lives — `<profile root>/<repo name from the url>`, the root being the profile's `CLAUDE_TASKS_ROOT` — verified before touching anything: the directory exists and `git -C <path> remote get-url origin`, normalized, equals `url`; a mismatch or missing checkout is a `Needs unblock` ask subtask (say which path was expected), never a reason to clone or pick a look-alike. A note without `url` is a repo-less project (next section). A *repo* without a matching note is the one lookup that asks instead of guessing: the unknown is which Todoist project the repo maps to, and guessing that wrong misroutes tasks rather than just misplacing a file.

Whichever field it was found by, keep the note in hand: its folder is where task notes live (see Task notes in the vault), and its Development Logs hold project-level findings — unless the note is flagged `dev-log: per-task` (next paragraph).

**`dev-log: per-task`** — an optional third field, marking a catch-all project whose tasks are independent of one another (the user's top-level `life` and `work` lists, as opposed to a project that is one body of work). The project note still resolves exactly as above and its folder still holds the task notes; what changes is where writing goes. Its Development Logs are **never written**: there is no project-level story for entries to belong to, so the fallback of logging in the project note is off, and so is the "findings that outlive any single task" route — anything worth recording goes in the task note that produced it. Every task with something to hold gets its own task note at pickup, per Task notes in the vault (the "skip it for small autonomous tasks" case still applies, and then the task gets no note anywhere — not a line in the project note). The flag is the user's to set; when creating a project note on the spot, don't guess at it.
...
- The vault side is unchanged: task notes are peers of the project note, `# Decisions` and Development Logs work as usual. Dev-log entries route to the project note already in hand from resolution — don't hunt for it by `url` — or, when that note carries `dev-log: per-task`, to the task note only.
...
### Workspaces: the checkout or a worktree

Where a task's files live while I work on them is the profile's `CLAUDE_TASKS_WORKSPACE`:

- **`checkout`** (personal): work in the verified checkout, `<root>/<repo>`, branching there as always. Nothing else shares that tree on the personal machine.
- **`worktree`** (work): the checkout is the user's own development environment — the one that can run the UI — and the user is often in it. Tasks stay out of it: each works in a **temporary git worktree**, created at pickup and removed when the round ends. Only a task the user has labeled **`claude-devenv`** works in the checkout itself. The label is the whole signal — no lease, no lock, no inference from the state of the tree — so the user grants the environment per task, when they know they won't need it, and takes it back by removing the label or reassigning the task.

Whichever workspace a task has, the `Phase:` block carries a `Workspace:` line while the task is In progress (see Multi-phase tasks), and the workspace is released — worktree removed, checkout restored — before every round ends: before an ask subtask is created, before the finishing report, and during a handback close-out. A task in Waiting holds nothing.

**Reading is free.** Under either default, investigation that only reads (a `claude-plan-required` task, the survey, resolving a project) may read the checkout as it is. Anything that runs code — tests, builds, reproductions — takes a workspace like any other work, because the checkout's tree may be mid-edit by the user.

#### Worktrees

- **Path**: `<CLAUDE_TASKS_WORKTREES>/<repo name>/<branch>` — the profile names the directory (`printenv CLAUDE_TASKS_WORKTREES` wins over the profile text, as with the root; the loop exports the resolved path). It sits under the root so headless firings reach it without another `--add-dir`; an interactive session started *inside* a checkout doesn't include it, so expect prompts there, or add it with `/add-dir <path>` once and say so in the report.
- **Create** from the checkout's git directory, never by cloning: `git -C <checkout> fetch origin`, then for a new branch `git -C <checkout> worktree add -b <branch> <path> origin/main`, or for a branch that already exists (a later round) `git -C <checkout> worktree add <path> <branch>` followed by `git -C <path> pull --ff-only origin <branch>` so the user's own pushes to the PR branch are included. If git refuses because the branch is checked out in the checkout itself, the user has it open there (reviewing it, say): don't force or detach — report that in one line, leave the task where it is, and take the next candidate; it's eligible again next round.
- **Work in it with absolute paths**, exactly as global mode works in a checkout: `git -C <path>`, `npm --prefix <path>`, every Glob/Grep anchored under `<path>`. Read the repo's `AGENTS.md`/`CLAUDE.md` from the worktree (same content, and it may say how to bootstrap one).
- **A worktree starts bare**: no installed dependencies, no untracked env files, no local database, no running services. Install what the tree can install (its lockfile, its build) and do everything the task allows from there. When the task genuinely needs what only the checkout has — the running UI, a local service, secrets that live outside the tree — finish everything that doesn't, then hand off with a `Needs unblock` ask subtask saying the task needs the development environment: adding `claude-devenv` and completing the ask is the answer, and the next round runs in the checkout.
- **Remove at the end of every round**, after the branch is pushed: `git -C <checkout> worktree remove <path>` (then `git -C <checkout> worktree prune`). Never remove a worktree holding uncommitted or unpushed work — if a push failed, leave it in place, keep the `Workspace:` line pointing at it, and say so in the report and the ask. Nothing is lost by removal otherwise: the branch lives on the remote and the next round recreates the worktree from it. The local branch stays in the checkout's refs, which is harmless.

#### The checkout (`checkout` default, or a `claude-devenv` task)

The user's state in the checkout is **carried along, not questioned** — they regularly leave uncommitted files behind, and a back-and-forth over them would cost more than it saves (user decision, 2026-09-11):

- **At pickup**, record the branch the checkout is on (`git -C <path> symbolic-ref --short HEAD`, or the commit if detached) in the `Workspace:` line — `Workspace: checkout (was on <branch>)` — then `git fetch origin` and create or switch to the task branch: `git switch -c <branch> origin/main` for a new one, `git switch <branch>` plus `git pull --ff-only origin <branch>` for an existing one. Untracked files and clean-applying modifications come across the switch on their own. Don't ask about them, don't report them, don't wait.
- **A problem is one of three things**: git refuses the switch because the user's changes would be overwritten; their changes touch files the task has to edit; or their changes break something the task has to verify (the suite, the build). Only then stash — `git -C <path> stash push --include-untracked -m "claude-tasks: <task id> <task title>"` — and record it: a `Stash: <message>` line in the `Phase:` block, and a sentence in the round's report and in whatever ask subtask the round ends with, saying what was stashed and how to get it back (`git stash list`, `git stash pop`). Stash the whole set: one entry, one pop.
- **Commit only by path** — `git add <file>…`, never `-a`, `-A`, or `git add .` — since the user's edits sit in the same tree. **Before pushing**, check `git diff --name-only origin/main...HEAD` lists only the task's own files; anything of the user's that slipped in comes out before the push.
- **At the end of every round** — before the ask subtask is created, before the finishing report, in a handback close-out, and after any failure that ends the turn — return the checkout to the recorded branch (`git switch <branch>`) and pop the task's own stash if it took one (`git stash pop` of the entry whose message names the task). The pop lands on the same commit the stash was taken from, so it applies cleanly; if it doesn't, `git reset --hard` back to a clean tree, leave the stash entry in place (a failed pop never drops it), and say so in Todoist. Then drop the `Workspace:` and `Stash:` lines. What was never stashed is still there, where the user left it.
- **Two labeled tasks can't share the checkout.** A `claude-devenv` candidate is not eligible this round while another task on the same repo is In progress with `Workspace: checkout` in its `Phase:` block — a concurrent interactive session has the tree. Report it in one line and take the next candidate. A *stale* In progress task with that line (see the stale check) is the abandoned session's; restoring the checkout — recorded branch, its stash — is the first step of resuming it.

Under the `checkout` default the same rules apply to every task, since the user may be in that tree too; there is simply no worktree step unless a task needs a clean tree (main is broken in the checkout, a dependency install that would disturb it), in which case it may take one as above and say so.
...
**Branching:** branch off up-to-date `main`, so each PR stands independent. When that doesn't make sense — the task builds on an unmerged branch, or main is broken — call it out: say what I branched from and why in both the report and the PR description. Branch names: short and descriptive (e.g. `fix/shared-zone-body-lookup`). Where the branch is worked — the checkout or a temporary worktree — is Workspaces' call, and the checkout's rules on carrying the user's uncommitted state and committing only by path apply there.
...
```
td --user <bot> task add "Needs <kind>: <one-line summary>" \
  --parent id:<work-task-id> --project id:<project-id> \
  --assignee id:<user-uid> --stdin --json < body.md
```

**Who the user is** comes from the project, never from a remembered address — the two profiles have different users. `GET https://api.todoist.com/api/v1/projects/<id>/collaborators` (with my token, like the invitation calls) lists the members with `id`, `name`, and `email`; in a two-member project, the one that isn't me is the user. With more members (a coworker shared into a work project), the user is the account that assigned me the task — `assigned_by_uid` on `GET …/tasks/<id>` (td's JSON omits it) — and when that is me too (self-assigned from chat), ask in chat who the ask goes to rather than guessing. Resolve it once per project per session; it doesn't change.
...
The profile's `CLAUDE_TASKS_GITHUB_MODE` says whose GitHub account I act through, and that decides how authorship is shown and how a review round closes. Everything else about PRs — the standing grant, the gates, the writing-style rule, comment routing — is the same in both modes.

**`bot`** (the personal profile): I commit, comment, and open PRs as my own account (`CLAUDE_TASKS_GITHUB_LOGIN`, distinct from the user's `CLAUDE_TASKS_USER_GITHUB_LOGIN`). The author field carries authorship, so nothing I write needs an in-text marker; the user can answer a review by approving or merging on GitHub; and I put a PR back on their plate by re-requesting their review.

**`shared`** (the work profile): I act through the user's own GitHub login — a work restriction, so both logins in the profile are the same. The author field then says nothing, and four things change:

- **Every comment I post starts with `🤖 `** — the first line of every PR comment, review body, inline review comment, issue comment, and PR description, including the integration-test evidence comment and any later edit to a body I wrote. Once, at the very start (`🤖 Fixed in abc123: …`), never mid-text. The user never starts a comment that way, so under the shared login: prefixed = mine, unprefixed = the user's. That is also how I scan for unaddressed comments — an unprefixed comment under the shared login is the user's and is owed an answer.
- **Commits carry no prefix.** The author is whatever the work tree's git configuration says (the user's, per their settings for that tree); the `Co-Authored-By` trailer Claude Code adds is the marker there. Don't prefix commit subjects — it breaks `git log` conventions and changelogs.
- **No re-requesting review.** GitHub refuses a review request for the PR's author, who is now the user. The Todoist ask subtask is the only channel: when a round ends (fixes pushed, every comment answered), leave a comment on the open `Needs review` ask — one line, what changed and the PR link — so it surfaces in Todoist again, then stay Waiting.
- **No approval on GitHub.** An author can't approve their own PR, so of Approval on GitHub only *merged* counts as a GitHub-side answer. A request-changes-style instruction arrives as an unprefixed comment and routes as usual.

Two consequences worth stating. *"A repo the user owns"* (the standing PR grant) means, in `shared` mode, a repository the user pushes to and opens PRs on directly in their normal work — not one they contribute to from a fork: the acts are literally theirs, so the test is whether they'd do that act there unprompted. And a merge that branch protection refuses (required approvals from others, a work rule) is a `Needs unblock` ask, never something to route around.

Whichever the mode, the profile is read before the first GitHub write of a session. A firing that can't tell which mode it's in — no profile readable — does no GitHub write at all; a `Needs unblock` ask instead.

## Responding to PR comments

A PR under review keeps generating input after its `Needs review` ask subtask exists — from the user, or anyone else with access to the repo. Check for it wherever the ask subtask itself gets checked (see Finding and choosing the next task, step 5, and Loop mode): `gh pr view <n> --comments` for the conversation thread, `gh api repos/<owner>/<repo>/pulls/<n>/comments` for inline review comments. A comment counts as unaddressed until a later reply of mine actually answers it — not merely any reply of mine that happens to sit after it in the thread. How I tell my own past replies from everyone else's depends on the GitHub identity mode: in `bot` mode the author field (shown by both `gh pr view` and the API) is mine vs. theirs; in `shared` mode the author field is the user's login either way and the 🤖 prefix is the marker — prefixed comments are mine, unprefixed ones under that login are the user's; but the position of a reply is only a hint, and each comment gets checked on its own content. Two comments from the user followed by one reply of mine that covers only the first leaves the second unaddressed. A reply that answers several comments at once says so explicitly, naming each point it covers, so the next check can see what was handled.

**Every comment from the user gets a response.** Whether it arrives inline, in a review body (approving, requesting changes, or comment-only), or as a plain conversation comment, a comment from the user (`CLAUDE_TASKS_USER_GITHUB_LOGIN` — in `shared` mode, one under that login *without* the 🤖 prefix) is never left hanging: I fix it, justify the existing behavior, answer the question, file it as a task — whatever fits — and reply so the thread shows it was handled. A comment of theirs I haven't replied to is an open item that keeps the round from ending. Other people's comments get the same routing, but only the user's are owed an answer.
...
In `bot` mode replies need no `Claude`/🤖 prefix or other in-text marker: everything I post to GitHub lands under my own account, so authorship is already visible on every comment, review, commit, and PR. In `shared` mode every reply starts with the 🤖 prefix — see GitHub identity modes.

**When the round ends and the PR needs the user's eyes again, re-request their review** — after replying to (and, where it applies, pushing fixes for) everything of theirs, not after each individual reply: `gh pr edit <n> --add-reviewer <user login>` (`POST /repos/<owner>/<repo>/pulls/<n>/requested_reviewers`; GitHub treats a request for someone who has already reviewed as a re-request). That is what puts the PR back in the user's review queue; a reply alone doesn't. It works in `bot` mode because the PR's author is my account, not the user — GitHub refuses to request a review from the author, which is exactly why `shared` mode can't do this: there, the round ends with a comment on the open ask subtask instead (see GitHub identity modes). In `bot` mode this holds on every route: a `Request changes` review, a comment-only review, plain conversation comments, and the comment-resolution step of Approval on GitHub when a change I pushed needs a fresh approval. Skip it only when nothing is left for the user to look at — the approval is current and I'm merging, or the PR is already merged. Re-requesting review is part of the standing PR grant on repos the user owns; elsewhere it's the same `Needs decision` hand-off as the reply itself.
...
The user can answer a `Needs review: PR <n>` ask from GitHub instead of Todoist. Read the PR's reviews wherever the ask gets checked (`gh pr view <n> --json reviews,mergedAt,headRefOid`, or `gh api repos/<owner>/<repo>/pulls/<n>/reviews`). Two signals count, and only from the user's own account (`CLAUDE_TASKS_USER_GITHUB_LOGIN` — another collaborator's approval is welcome but doesn't close the user's review). In `shared` mode only the first exists: the PR is the user's own, and GitHub doesn't let an author approve their own PR.
...
- skip it for small autonomous tasks with nothing to plan or decide — log durable findings in the project note's Development Logs instead (in a `dev-log: per-task` project there is no such fallback: if a finding is worth keeping, that is the cue to create the task note).
...
- **Sections**:
  ...
  - `# Development Logs` — per the **dev-log** skill. With a task note in play, entries about *this task's* work go here; the project note's Development Logs remain for findings that outlive any single task — except in a `dev-log: per-task` project, where everything goes here and the project note's log is never touched.
- **`todoist-task` frontmatter — set at creation**: the task's Todoist URL, verbatim from the CLI (see Linking to Todoist tasks). It mirrors `todoist-project` on project notes and is how a task note is found from its task — `search_query` on `{"==": [{"var": "frontmatter.todoist-task"}, "<url>"]}` — so check by this field before falling back to the filename when looking for a prior attempt's note. It is also the only index a `dev-log: per-task` project has: its project note keeps no list of task notes.
...
**On starting (from Queued):** read the repo's `AGENTS.md` if it has one — the user keeps project instructions there, and Claude Code does *not* load it automatically (only `CLAUDE.md`), so unless a `CLAUDE.md` imports it (`@AGENTS.md`) or symlinks to it, those instructions reach me only by reading the file deliberately. Then re-fetch comments — new guidance may have landed since the survey. Set the `Phase:` line (the claim), add `claude-inflight` (fetch first; preserve every other label), and create the task note if warranted (with its `todoist-task` frontmatter). Then take the task's workspace — a temporary worktree, or the checkout for a `claude-devenv` task or under the `checkout` default — and record it in the `Phase:` block (see Workspaces).

**On needing the user:** release the workspace first — push the branch and remove the worktree, or restore the checkout (see Workspaces) — then create the ask subtask (assigned to them), update `Phase:` (including the `Ask:` line, and without the `Workspace:` line), remove `claude-inflight`, report, stop.
...
2. **Integration-test evidence comment**: run the project's live/integration suite on the branch (for icloud-md: `ICLOUD_MD_ITEST=1 npm run test:integration`) and post a PR comment with the results (🤖-prefixed in `shared` GitHub mode). Concrete numbers from the run, corroborating evidence, an explicit accounting of any new risks and what bounds each, and the honest gaps. If the suite fails or can't run, say so in the comment and the report — never skip silently.
...
4. **Release the workspace** — the branch is pushed, so remove the worktree, or restore the checkout to the user's branch and pop any stash (see Workspaces).
5. **Hand off for review** — always, PR or not: a `Needs review` ask subtask, update `Phase:` to `delivered — PR <url>` (or the deliverable's link). When the user completes the ask with no reply or an approving one ("looks good"), or approves or merges the PR on GitHub (see Approval on GitHub), complete the work task — closing my own tasks is in scope; the user reviews the deliverable, not the task. Any other reply (review comments to address, a change of direction) is the next round's instruction, and the work continues on the same branch and PR.
6. **Report, then pause**: summarize what was done, the PR link and the `obsidian://` link to the task note (see Linking to vault notes), what to look at (point at `# Decisions` first), anything surprising — and, if user state was stashed in the checkout, what and how to get it back — then stop and invite review before continuing.

**When blocked or the premise fails:** release the workspace, then a `Needs unblock` or `Needs decision` ask subtask as appropriate, report. Never leave a task claimed-but-askless at the end of a turn unless I'm genuinely mid-work and about to continue — and never leave a workspace held past the end of a turn except in that same case.
