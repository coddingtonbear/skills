...
- **`claude-needs-you`** — never on a work task: the things I've handed to the user. Almost always an *ask subtask*; the one standalone exception is the fire-and-forget `Needs review: project note placement` task (see Finding and choosing the next task), where completion answers nothing — the note either stays or gets moved. Open = awaiting them; completed = answered. This is the user's inbox.
...
2. **Resolve each task's project**: list id → list name (`list_projects`) → the vault note whose `ticktick-list` frontmatter matches (`search_query` on `{"==": [{"var": "frontmatter.ticktick-list"}, "<name>"]}`) → that note's `url`, if it has one (a note without `url` is a repo-less project — see below). A task whose list has no project note *at all* gets one created on the spot — best guess, no waiting on the user. Pick the folder by analogy with the existing project notes (a sibling of the most similar projects), write the note with `ticktick-list` frontmatter — plus `url` (and `path` if non-standard) only when a repository is evident from the list or its tasks; otherwise it's a repo-less project — and carry on resolving as normal. Then flag the guess: create **one top-level task in that same list**, tagged `claude-needs-you` and nothing else, titled `Needs review: project note placement for <list>`, its body linking the new note (per Linking to vault notes) and stating what was guessed (folder; repo or none). It's fire-and-forget — no task goes to Waiting over it and nothing resumes when it's completed; if the guess was wrong the user just moves the note, and the mapping follows it, since resolution is by `ticktick-list` frontmatter, never by path. Create the note once, not per task — re-run the `ticktick-list` search before creating in case an earlier firing already did.
...
A project note may carry `ticktick-list` and no `url` — a project with no repository behind it (career management, planning, pure research). The list↔note association stands on `ticktick-list` alone, so these resolve normally in global mode; only the repo mechanics fall away:

- Skip the checkout steps (3–5 above): there is no checkout to resolve or verify and no repo instructions to read. In-repo mode can never surface such a project — there's no repo to start a session from — so its tasks are reached in global mode or by the user naming them in chat.
