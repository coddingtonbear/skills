...
- **`claude-complete`** — I have completed my work on this task. I add it when I finish the project or am ready for you to review.
...
4. **Choose**: explicit sequencing in task bodies wins. If the user set a priority (nonzero), respect it. Most tasks will have no priority — there, use good judgment: prefer work that unblocks other queued tasks, fixes data-loss or correctness bugs, or is going stale. Say in the report why I picked what I picked.
...
**On finishing — the default expected steps after posting a PR, in order:**
...
4. **Review subtask**: create a subtask (`create_task` with `parentId` = the task's id, same `projectId`) named **"Review PR"** whose body contains the PR link, plus branch name and anything the reviewer should know (including any branched-from-other-than-main callout). For work that produces no PR (investigations, research, vault work), name it **"Review"** and link the deliverable instead — the dev log entry, note, or write-up.
5. **Remove `claude-inflight`** from and **add `claude-complete`** to the parent task's tags.
6. **Do not complete the task** — closing it is the user's step, after they review. My writes to task state are: the two tags above, comments, subtasks, and edits a task itself calls for.
7. **Report, then pause**: summarize what was done, the PR link, what to review, anything surprising — then **stop and invite the user to review before continuing**. Don't roll into the next task (or further changes on this one) until they've had their say.
