...
2. **Resolve each task's project**: list → project note → repository, per Project resolution below.
...
How a TickTick list maps to a project note, and onward to a repository:

1. **List → name**: `list_projects` turns the list id into its name.
2. **Name → note**: the vault note whose `ticktick-list` frontmatter matches — `search_query` on `{"==": [{"var": "frontmatter.ticktick-list"}, "<name>"]}`. The association stands on `ticktick-list` alone, never on the note's path, so a moved note keeps resolving. Keep the note in hand: its folder is where task notes live (see Task notes in the vault), and its Development Logs hold project-level findings.
3. **No note? Create one — best guess, no waiting on the user.** Pick the folder by analogy with the existing project notes (a sibling of the most similar projects) and write the note with `ticktick-list` frontmatter, plus `url` (and `path` if non-standard) only when a repository is evident from the list or its tasks. Create it once, not per task: re-run the frontmatter search first, in case an earlier firing already did. Then flag the guess: one **top-level task in that same list**, tagged `claude-needs-you` and nothing else, titled `Needs review: project note placement for <list>`, its body linking the new note (per Linking to vault notes) and stating what was guessed (folder; repo or none). It's fire-and-forget — nothing goes to Waiting over it and nothing resumes when it's completed; a wrong guess is fixed by moving the note.
4. **Note → repository**: the note's `url` frontmatter names the repo; no `url` means a repo-less project (next section).

In-repo mode's step 1 is this same chain run backwards — repo → note via `url` frontmatter → `ticktick-list`. The creation step doesn't apply there: with no matching note, the unknown is which TickTick list the repo maps to, and guessing that wrong misroutes tasks rather than just misplacing a file — so ask instead.
...
- Skip global mode's checkout steps (3–5): there is no checkout to resolve or verify and no repo instructions to read. In-repo mode can never surface such a project — there's no repo to start a session from — so its tasks are reached in global mode or by the user naming them in chat.
